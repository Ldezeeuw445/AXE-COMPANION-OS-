...
import { readPref, seedGlobalsFromAccount, writePref } from "@/lib/accountPreferences";
import { chartPreferenceEvent } from "@/lib/adaptive/eventTypes";
import { trackAdaptiveEvent } from "@/lib/adaptive/trackAdaptiveEvent";
import type { AdaptiveSuggestionState, AdaptiveUiDecisionSet } from "@/types/adaptive";
import {
  AxeContextToolbar,
...
export function ChartScreen({ data, initialAction, liveTradingEnabled = false, instantSlTpModify = false }: Props) {
  ...
  const adaptiveSessionIdRef = useRef<string>(
  ...
  const [adaptiveSuggestions, setAdaptiveSuggestions] = useState<AdaptiveSuggestionState[]>([]);
  ...
  useEffect(() => {
    try {
      const raw = Number(readPref(accountId, "axe.chart.obCount") ?? "");
      if (raw === 1 || raw === 2 || raw === 3) setOrderBlockCount(raw);
      const rawIfvg = Number(readPref(accountId, "axe.chart.ifvgCount") ?? "");
      if (rawIfvg === 1 || rawIfvg === 2 || rawIfvg === 3) setInverseFvgCount(rawIfvg);
      const rawFvg = Number(readPref(accountId, "axe.chart.fvgCount") ?? "");
      if (rawFvg === 1 || rawFvg === 2 || rawFvg === 3) setFvgCount(rawFvg);
      const rawProj = Number(readPref(accountId, "axe.chart.projectionCount") ?? "");
      if (rawProj === 1 || rawProj === 2 || rawProj === 3) setProjectionCount(rawProj);
      const rawMaPeriod = Number(readPref(accountId, "axe.chart.maPeriod") ?? "");
      if (rawMaPeriod === 9 || rawMaPeriod === 20 || rawMaPeriod === 50 || rawMaPeriod === 200) setMaPeriod(rawMaPeriod);
      const rawMaType = readPref(accountId, "axe.chart.maType");
      if (rawMaType === "sma" || rawMaType === "ema") setMaType(rawMaType);
      const rawIndicatorFlags = readPref(accountId, "axe.chart.indicatorFlags");
      ...
      const rawModeFlags = readPref(accountId, "axe.chart.modeFlags");
      if (rawModeFlags) {
        const parsed = JSON.parse(rawModeFlags) as Record<string, unknown>;
        setActiveToolFlags(Object.fromEntries(
          Object.entries(parsed).map(([key, value]) => [key, Boolean(value)]),
        ));
      } else {
        setActiveToolFlags({});
      }
      const rawFib = readPref(accountId, "axe.chart.fibMode");
      ...
      const rawFibSwing = Number(readPref(accountId, "axe.chart.fibSwingOffset") ?? "");
      if (rawFibSwing === 0 || rawFibSwing === 1 || rawFibSwing === 2 || rawFibSwing === 3) {
        setFibSwingOffset(rawFibSwing);
      }
      const rawFutureCursor = readPref(accountId, "axe.chart.futureCursorEnabled");
      if (rawFutureCursor === "0" || rawFutureCursor === "1") {
        setFutureCursorEnabled(rawFutureCursor === "1");
      }
      const rawPinnedQuickActions = readPref(accountId, "axe.chart.pinnedQuickActions");
      if (rawPinnedQuickActions) {
        const parsed = JSON.parse(rawPinnedQuickActions) as unknown;
        if (Array.isArray(parsed)) {
          setPinnedQuickActions(parsed.filter((item): item is string => typeof item === "string").slice(0, 5));
        }
      }
    } catch {
  ...
  useEffect(() => {
    const params = new URLSearchParams();
    params.set("scope", "chart");
    if (accountId) params.set("accountId", accountId);
    void (async () => {
      try {
        const res = await fetch(`/api/adaptive/suggestions?${params.toString()}`, { credentials: "include" });
        const json = (await res.json()) as { suggestions?: AdaptiveSuggestionState[] };
        const nextSuggestions = Array.isArray(json.suggestions) ? json.suggestions : [];
        setAdaptiveSuggestions(nextSuggestions);
        for (const suggestion of nextSuggestions) {
          void trackAdaptiveEvent({
            eventType: "adaptive_suggestion_shown",
            accountId,
            route: "/chart",
            sessionId: adaptiveSessionIdRef.current,
            payload: {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(`/api/adaptive/decisions?${params.toString()}`, { credentials: "include" });
        const json = (await res.json()) as { decisions?: AdaptiveUiDecisionSet };
        const decisions = json.decisions;
        if (!decisions) return;

        if (!readPref(accountId, "axe.chart.modeFlags")) {
          const defaultMode = decisions.chart.chartModes[0];
          if (defaultMode) {
            setActiveToolFlags((prev) => ({ ...prev, [defaultMode]: true }));
          }
        }
        if (!readPref(accountId, "axe.chart.pinnedQuickActions") && decisions.chart.topQuickActions.length > 0) {
          setPinnedQuickActions(decisions.chart.topQuickActions.slice(0, 5));
        }
        if (!readPref(accountId, "axe.chart.fibMode")) {
          const fibMode = decisions.chart.fibMode;
          if (fibMode === "auto" || fibMode === "swing" || fibMode === "pd" || fibMode === "sd") {
            setFibMode(fibMode);
          }
        }
      } catch {
  ...
  const updateFibMode = useCallback((next: FibMode) => {
    ...
    void trackAdaptiveEvent({
      ...chartPreferenceEvent("chart_fib_adjusted", accountId, {
        mode: next,
  ...
  }, [accountId, data.symbol, data.timeframeKey, fibSwingOffset, savePref]);
  ...
  }, [accountId, data.symbol, data.timeframeKey, fibMode, savePref]);
  ...
  useEffect(() => {
    try {
      const v = Number(readPref(accountId, "axe.chart.paneHeight.volume") ?? "");
      const r = Number(readPref(accountId, "axe.chart.paneHeight.rsi") ?? "");
      const m = Number(readPref(accountId, "axe.chart.paneHeight.macd") ?? "");
      setPaneHeights((prev) => ({
  ...
  useEffect(() => {
    try {
      const stored = readPref(accountId, "axe.chart.paneOrder");
      if (stored) {
  ...
  useEffect(() => {
    void trackAdaptiveEvent({
      ...chartPreferenceEvent("chart_opened", accountId, {
        symbol: data.symbol,
        timeframe: data.timeframeKey,
        source: data.source,
      }),
  ...
  }, [accountId, data.symbol, data.timeframeKey, data.source]);
  ...
  const goSymbol = useCallback(
    (sym: string) => {
      void trackAdaptiveEvent({
        ...chartPreferenceEvent("chart_symbol_selected", accountId, {
          symbol: sym,
          timeframe: data.timeframeKey,
        }),
        sessionId: adaptiveSessionIdRef.current,
    ...
    [router, accountId, data.timeframeKey, savePref, startRouteTransition],
  );
  const goTf = useCallback(
    (key: string) => {
      void trackAdaptiveEvent({
        ...chartPreferenceEvent("chart_timeframe_selected", accountId, {
          symbol: data.symbol,
          timeframe: key,
        }),
        sessionId: adaptiveSessionIdRef.current,
      ...
      setRouteFallbackMessage(null);
      try {
        savePref("axe.chart.timeframe", key);
      } catch {
        /* ignore */
      }
      startRouteTransition(() => {
    ...
    [router, accountId, data.symbol, savePref, startRouteTransition],
  );
  ...
  const toggleAutoAnnotation = useCallback(
    (kind: "fib_retracement" | "trendline") => {
      ...
      if (kind === "fib_retracement") {
        void trackAdaptiveEvent({
          ...chartPreferenceEvent("chart_fib_drawn", accountId, {
            mode: fibMode,
            swingOffset: fibSwingOffset,
            symbol: data.symbol,
            timeframe: data.timeframeKey,
    ...
    [accountId, annotations, data.symbol, data.timeframeKey, executeActionByType, fibMode, fibSwingOffset],
  );
  ...
  const toggleToolFlag = useCallback((id: string) => {
    setActiveToolFlags((prev) => {
      ...
      const nextFlags = { ...prev, [id]: nextEnabled };
      void trackAdaptiveEvent({
        ...chartPreferenceEvent(
          nextEnabled ? "chart_mode_enabled" : "chart_mode_disabled",
          accountId,
          {
            mode: id,
            symbol: data.symbol,
      ...
      try {
        savePref("axe.chart.modeFlags", JSON.stringify(nextFlags));
      } catch {
        /* ignore */
      }
      return nextFlags;
    });
  }, [accountId, data.symbol, data.timeframeKey, savePref]);

  const toggleIndicatorFlag = useCallback((id: string) => {
    setIndicatorToolFlags((prev) => {
      const next = { ...prev, [id]: !prev[id] };
      const nextEnabled = next[id];
      void trackAdaptiveEvent({
        ...chartPreferenceEvent(
          nextEnabled ? "chart_indicator_enabled" : "chart_indicator_disabled",
          accountId,
          {
            indicator: id,
            symbol: data.symbol,
  ...
  const recordQuickAction = useCallback((actionId: string) => {
    void trackAdaptiveEvent({
      ...chartPreferenceEvent("chart_quick_action_used", accountId, {
        actionId,
        symbol: data.symbol,
        timeframe: data.timeframeKey,
      }),
  ...
  const toggleFutureCursor = useCallback(() => {
    setFutureCursorEnabled((prev) => {
      const next = !prev;
      try {
        savePref("axe.chart.futureCursorEnabled", next ? "1" : "0");
      } catch {
        /* ignore */
      }
      void trackAdaptiveEvent({
        ...chartPreferenceEvent(
          next ? "chart_mode_enabled" : "chart_mode_disabled",
          accountId,
          {
            mode: "futureCursor",
            symbol: data.symbol,
  ...
  const resolveAdaptiveSuggestion = useCallback(async (suggestion: AdaptiveSuggestionState, action: "accept" | "dismiss") => {
    if (action === "accept") {
      if (suggestion.kind === "fib_style_default") {
        const mode = typeof suggestion.payload.mode === "string" ? suggestion.payload.mode : null;
        if (mode === "auto" || mode === "swing" || mode === "pd" || mode === "sd") {
          updateFibMode(mode);
        }
      }
      if (suggestion.kind === "chart_mode_default") {
        const mode = typeof suggestion.payload.mode === "string" ? suggestion.payload.mode : null;
        if (mode) {
          setActiveToolFlags((prev) => {
            const next = { ...prev, [mode]: true };
            try {
              savePref("axe.chart.modeFlags", JSON.stringify(next));
            } catch {
              /* ignore */
            }
            return next;
      ...
      if (suggestion.kind === "quick_action_pin") {
        const actions = Array.isArray(suggestion.payload.actions)
          ...
          : [];
        setPinnedQuickActions(actions);
        try {
          savePref("axe.chart.pinnedQuickActions", JSON.stringify(actions));
        } catch {
          /* ignore */
        }
      }
    ...
    setAdaptiveSuggestions((prev) => prev.filter((item) => item.id !== suggestion.id));
    void trackAdaptiveEvent({
  ...
  }, [accountId, savePref, updateFibMode]);

  const chartToolItems = useMemo(() => ([
    ...
    { id: "futureCursor", label: "Project", icon: MoveHorizontal, active: futureCursorEnabled, action: toggleFutureCursor },
  ]), [activeToolFlags, data.symbol, futureCursorEnabled, hasFibAnnotation, hasTrendAnnotation, router, tfLabel, toggleAutoAnnotation, toggleFutureCursor, toggleToolFlag, flipFibAnnotation]);
    ...
    >
      ...
      >
        ...
        {adaptiveSuggestions.length > 0 ? (
          <div className="absolute left-3 right-3 top-[4.8rem] z-30 max-w-[34rem]">
            <AdaptiveSuggestionStack
              title="AXE can make this chart feel more like yours"
              suggestions={adaptiveSuggestions}
              onAccept={(suggestion) => resolveAdaptiveSuggestion(suggestion, "accept")}
              onDismiss={(suggestion) => resolveAdaptiveSuggestion(suggestion, "dismiss")}
            />
          </div>
        ) : null}
        ...
        >
          ...
          {pinnedQuickActions.length > 0 ? (
            <div className="mb-2">
              <div className={`mb-1 text-[8px] font-bold uppercase tracking-[0.18em] ${chartTheme.isDark ? "text-cyan-200/70" : "text-cyan-800/70"}`}>
                Quick picks
              </div>
              <div className="grid grid-cols-4 gap-1.5">
                {chartToolItems
                  .filter((item) => pinnedQuickActions.includes(item.id))
                  .slice(0, 4)
                  .map((item) => {
                        ...
                        className={`flex h-11 flex-col items-center justify-center rounded-xl border text-[10px] transition ${
                          isDisabled
                            ...
                            : item.active
                              ...
                              : chartTheme.isDark
                                ? "border-cyan-400/16 bg-cyan-400/[0.05] text-cyan-100/82 hover:text-white"
                                : "border-cyan-700/18 bg-cyan-500/[0.08] text-cyan-900/80 hover:text-cyan-950"
                        }`}
