...
export function ChartScreen({ data, initialAction, liveTradingEnabled = false }: Props) {
  ...
  const [pendingCancelArmed, setPendingCancelArmed] = useState(false);
  ...
  useEffect(() => {
    if (!pendingOrderVisible || executionMode !== "pending") {
      setPendingCancelArmed(false);
    }
  }, [pendingOrderVisible, executionMode]);
    ...
    >
      ...
      >
        ...
        {pendingOrderVisible && executionMode === "pending" ? (
          <>
            <TradePlanLine
              ...
              onSelect={() => setPendingCancelArmed(true)}
            ...
            <TradePlanLine
              ...
              onDragEnd={() => { vibrate("light"); playSound("tap"); }}
            />
            {/* Cancel only after selecting/tapping the pending entry line. */}
            {pendingCancelArmed ? (
              <button
                type="button"
                onClick={() => {
        ...
        <button
          ...
          className={`absolute left-0 top-[30%] z-40 grid h-16 w-6 -translate-y-1/2 place-items-center rounded-r-2xl border border-l-0 backdrop-blur transition ${
            toolRailOpen
              ? chartTheme.isDark
        ...
        <div
          className={`absolute left-0 top-[30%] z-30 max-h-[46vh] -translate-y-1/2 overflow-y-auto rounded-r-2xl border border-l-0 border-white/10 bg-black/82 p-2.5 shadow-[0_18px_60px_rgba(0,0,0,0.62)] backdrop-blur-xl transition-transform ${
