...
export function ChartScreen({ data }: Props) {
  ...
  const onPositions = useCallback(
    ({ total, onSymbol }: { total: number; onSymbol: LivePosition[] }) => {
  ...
  // queueMicrotask is fired after render, so it does not loop the renderer.
  ...
  const lastPriceText = useMemo(() => formatBrokerPrice(data.brokerSymbol, livePrice), [data.brokerSymbol, livePrice]);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4 pb-4">
      <ScreenHeader
        title={data.symbol}
        subtitle={`${tfLabel} · Broker OHLC via MetaApi MT5${accountLabel ? ` · ${accountLabel}` : ""}`}
        left={<LineChart className="h-6 w-6 text-cyan-400/85" aria-hidden />}
        right={
          <span
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 px-1">
        <div className="min-w-0">
          <p className="font-mono text-2xl font-semibold tracking-tight text-tos-text sm:text-3xl">{lastPriceText}</p>
          <p className="mt-1 text-[10px] uppercase tracking-[0.18em] text-tos-dim">
            {tfLabel} close · MetaApi MT5
            {data.lastCandleTime ? ` · ${formatBrokerTime(data.lastCandleTime)}` : ""}
          </p>
        </div>
        {liveSummary ? (
          <span className="rounded-full border border-white/10 bg-white/[0.03] px-2.5 py-1 text-[10px] uppercase tracking-wider text-tos-muted">
            {liveSummary}
          </span>
        ) : null}
      </div>
      ...
      {/* Empty / failed states sit ABOVE the chart so the chart frame still loads */}
      {failureCopy ? (
        <GlassPanel
          className="border-amber-400/15 p-4"
          glow={data.failure === "candles_unavailable" ? "warm" : "none"}
        >
      ...
      {/* Position summary (current symbol) */}
      {overlays.length > 0 ? (
        <GlassPanel className="!p-3">
          ...
          <ul className="space-y-2">
            {overlays.map((o) => {
              const distSL =
                o.stopLoss != null && o.currentPrice != null
                  ? Math.abs(o.currentPrice - o.stopLoss)
                  : null;
              const distTP =
                o.takeProfit != null && o.currentPrice != null
                  ? Math.abs(o.takeProfit - o.currentPrice)
                  : null;
              const rr = (() => {
              ...
              const profitColor = profit > 0 ? CHART_THEME.positiveText : profit < 0 ? CHART_THEME.negativeText : CHART_THEME.neutralText;
              ...
              return (
                ...
                >
                  ...
                  {rr != null ? (
                    <span className="font-mono text-[11px] text-tos-dim">RR {rr.toFixed(2)}</span>
                  ) : null}
                  {o.openTime ? (
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        ...
        <Link
          ...
          className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5 text-[11px] font-medium text-tos-muted transition-colors hover:border-cyan-500/30 hover:text-tos-text sm:col-span-1"
        >
          <Sparkles className="h-4 w-4 shrink-0 text-cyan-400/85" />
          Trade plan (intent)
        </Link>
        <span className="hidden items-center gap-2 rounded-xl border border-white/[0.04] bg-white/[0.015] px-3 py-2.5 text-[11px] font-medium text-tos-dim sm:flex">
          <Cpu className="h-4 w-4 shrink-0 text-tos-dim/80" />
          Execution disabled by default
        </span>
      </div>

      {/* Data details */}
      <details className="group overflow-hidden rounded-2xl border border-white/[0.07] bg-black/25" open={data.failure !== "ok"}>
        ...
        <dl className="grid grid-cols-1 gap-x-4 gap-y-2 border-t border-white/[0.05] px-4 py-3 text-[11px] sm:grid-cols-2">
          ...
          <div>
            ...
            <dd className="font-mono text-tos-text">{tfLabel} ({data.metaApiTimeframe})</dd>
          </div>
          <div>
