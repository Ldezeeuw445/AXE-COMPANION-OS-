...
var CacheEngine = class {
   ...
   */
  async getOrFetch(key2, ttlMs, staleWhileRevalidate, fetcher) {
    const now = Date.now();
    ...
    if (cached && staleWhileRevalidate && now < cached.staleAt) {
      ...
      this.backgroundRefresh(key2, ttlMs, staleWhileRevalidate, fetcher);
      return { data: cached.data, state: "stale" };
    }
    this.stats.misses++;
    try {
      const data = await fetcher();
      this.set(key2, data, ttlMs, staleWhileRevalidate);
   ...
   */
  set(key2, data, ttlMs, staleWhileRevalidate = false) {
    const now = Date.now();
    if (this.cache.size >= this.maxSize) {
      const oldest = Array.from(this.cache.entries()).sort((a, b) => a[1].fetchedAt - b[1].fetchedAt)[0];
      if (oldest) {
  ...
  invalidate(pattern) {
    ...
    for (const [key2] of this.cache) {
      if (key2.includes(pattern)) this.cache.delete(key2);
    }
  }
  getStats() {
    return { ...this.stats, size: this.cache.size };
  }
  async backgroundRefresh(key2, ttlMs, staleWhileRevalidate, fetcher) {
    try {
      const data = await fetcher();
      this.set(key2, data, ttlMs, staleWhileRevalidate);
    } catch {
...
var InflightDeduper = class {
  ...
  async dedupe(key2, fetcher) {
    ...
    this.inflight.set(key2, { promise, timestamp: now });
    return promise;
  }
  getStats() {
...
// src/engine/core/router.ts
var SourceRouter = class {
  providers = [];
  health;
  stats = { selections: 0, fallbacksUsed: 0, fallbacksByDomain: {} };
  constructor(health) {
    this.health = health;
  }
  register(config) {
   ...
   */
  select(providerIds, priority) {
    const candidates = this.providers.filter((p) => providerIds.includes(p.id)).filter((p) => this.health.isHealthy(p.id)).filter((p) => p.usedThisMonth < p.monthlyLimit).filter((p) => p.dailyLimit === 0 || p.usedToday < p.dailyLimit);
   ...
   */
  buildFallbackChain(primaryId, allIds, enableFallback) {
  ...
  recordUsage(providerId, costUnits2) {
    const p = this.providers.find((x) => x.id === providerId);
    if (p) {
      const units = Math.max(0, Math.floor(costUnits2 ?? (p.costPerCall ?? 1)));
      p.usedThisMonth += units;
      p.usedToday += units;
    }
  }
  recordFallbackUsed(domain) {
  ...
  calculateScore(p, priority) {
    ...
    const costScore = 1 / (1 + Math.max(0, (p.costPerCall ?? 1) - 1) / 2);
    switch (priority) {
      case "CHEAP":
...
var ProviderHealthTracker = class {
  ...
  classifyRateLimit(error) {
    const msg = this.toMessage(error) ?? "";
    if (/\b429\b/.test(msg) || /too many requests/i.test(msg) || /rate limit/i.test(msg)) {
      return { reason: "rate_limited", cooldownMs: 5 * 6e4 };
    }
    if (/\b402\b/.test(msg) || /quota/i.test(msg) || /credit/i.test(msg) || /payment required/i.test(msg)) {
      return { reason: "quota_exhausted", cooldownMs: 60 * 6e4 };
    }
    return null;
  }
};

// src/engine/core/metrics.ts
var MetricsCollector = class {
  history = [];
  maxPoints;
  lastSnapshot = 0;
  snapshotIntervalMs;
  constructor(maxPoints = 1e3, snapshotIntervalMs = 6e4) {
   ...
   */
  record(cacheHitRate, creditSavingsRate, avgLatencyMs, activeProviders) {
    const now = Date.now();
...
var Normalizer = class {
  ...
  static dateRange(input) {
    ...
    const from = new Date(now.getTime() - days * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
    return { from, to };
  }
  /**
...
async function withRetry(fn, opts) {
  ...
  for (let attempt = 1; attempt <= opts.maxAttempts; attempt++) {
    try {
      ...
      const j = capped * jitter * (Math.random() * 2 - 1);
...
// src/engine/core/cost.ts
function costUnits(input) {
...
// src/engine/services/newsService.ts
var NewsService = class {
  ...
  providers;
  policy;
  constructor(cache, deduper, router, health, configs) {
    this.cache = cache;
  ...
  async getNews(filter = {}) {
    const symbol = filter.symbol ? Normalizer.symbol(filter.symbol) : void 0;
   ...
   */
  async fetchFromProviders(symbol, filter) {
  ...
  normalizeNews(raw, symbol) {
    ...
    return raw.map((item, i) => {
      ...
      return {
        ...
        summary: item.text || item.summary || item.description || void 0,
        url: item.url || item.link || void 0,
        publishedAt: item.publishedDate || item.published_at || item.datetime || (/* @__PURE__ */ new Date()).toISOString(),
...
// src/engine/providers/fred.ts
var BASE_URL = "https://api.stlouisfed.org/fred";
var FREDProvider = class {
  apiKey;
  constructor(config) {
   ...
   */
  async fetchSeries(seriesId, from, to) {
    ...
    const url2 = `${BASE_URL}/series/observations?${params}`;
  ...
  async fetchSeriesInfo(seriesId) {
    const url2 = `${BASE_URL}/series?series_id=${seriesId}&api_key=${this.apiKey}&file_type=json`;
    const res = await fetch(url2);
...
// src/engine/services/macroService.ts
var SERIES_MAP = {
...
function inferFredSeriesMeta(key2) {
  const k = key2.trim();
...
var MacroService = class {
   ...
   */
  async getSeries(key2, filter = {}) {
    const seriesInfo = SERIES_MAP[key2] ?? inferFredSeriesMeta(key2);
    if (!seriesInfo) {
      throw new Error(`Unknown macro series: ${key2}`);
    }
    const range = Normalizer.dateRange(filter.range);
    const cacheKey = Normalizer.cacheKey("macro", { key: key2, from: range.from, to: range.to });
    const result = await this.cache.getOrFetch(
      cacheKey,
      this.policy.cacheTtlMs,
      this.policy.staleWhileRevalidate || false,
      () => this.deduper.dedupe(cacheKey, () => this.fetchFromProviders(key2, range.from, range.to))
    );
    return result.data;
  }
  /**
   ...
   */
  listAvailableSeries() {
    return Object.entries(SERIES_MAP).map(([key2, info]) => ({
      key: key2,
      name: info.name,
      unit: info.unit,
   ...
   * Fetch from providers with failover.
   */
  async fetchFromProviders(key2, from, to) {
    const seriesInfo = SERIES_MAP[key2] ?? inferFredSeriesMeta(key2);
    if (!seriesInfo) throw new Error(`Unknown macro series: ${key2}`);
    const providerIds = Array.from(this.providers.keys());
    const primaryId = this.router.select(providerIds, this.policy.priority);
    if (!primaryId) {
      ...
      try {
        ...
        const raw = await withRetry(
          async () => {
            ...
            return await config.provider.fetchMacro(key2, from, to);
          },
          { maxAttempts: 2, baseDelayMs: 200, maxDelayMs: 1200 }
        );
        this.health.recordSuccess(id);
        return this.normalizeMacro(key2, raw.value, seriesInfo);
      } catch (e) {
        lastError = e;
        this.health.recordFailure(id, e);
      }
   ...
   */
  normalizeMacro(key2, raw, info) {
    const observations = raw.observations || [];
    const points = observations.map((obs) => ({
      date: obs.date,
      value: obs.value === "." ? NaN : parseFloat(obs.value)
    })).filter((p) => !isNaN(p.value)).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
...
// src/engine/services/accountService.ts
  ...
  async fetchSummary(userId) {
    ...
    try {
      ...
      return {
        ...
        lastUpdated: raw.updated_at || (/* @__PURE__ */ new Date()).toISOString()
      };
    } catch (e) {
...
// src/engine/types/chart.ts
var ChartFetchError = class extends Error {
  debug;
...
// src/engine/providers/polygon.ts
var BASE_URL2 = "https://api.polygon.io/v2";
var PolygonProvider = class {
   ...
   */
  async fetchAggregates(symbol, multiplier = 1, timespan = "day", from, to) {
    const fromDate = from || this.daysAgo(30);
    const toDate = to || this.today();
    const url2 = `${BASE_URL2}/aggs/ticker/${symbol}/range/${multiplier}/${timespan}/${fromDate}/${toDate}?apiKey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  async fetchQuote(symbol) {
    const url2 = `https://api.polygon.io/v3/quotes/${symbol}?apiKey=${this.apiKey}`;
    const res = await fetch(url2);
   ...
   */
  async fetchCrypto(pair) {
    const url2 = `https://api.polygon.io/v2/aggs/ticker/X:${pair}/prev?apiKey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  daysAgo(n) {
    return new Date(Date.now() - n * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
  }
};

// src/engine/providers/twelvedata.ts
var BASE_URL3 = "https://api.twelvedata.com";
var TwelveDataProvider = class {
   ...
   */
  async fetchQuote(symbol) {
    const url2 = `${BASE_URL3}/quote?symbol=${encodeURIComponent(symbol)}&apikey=${encodeURIComponent(this.apiKey)}`;
    const res = await fetch(url2);
   ...
   */
  async fetchTimeSeries(symbol, interval = "1min", outputsize = 100) {
    const url2 = `${BASE_URL3}/time_series?symbol=${encodeURIComponent(symbol)}&interval=${encodeURIComponent(interval)}&outputsize=${encodeURIComponent(String(outputsize))}&apikey=${encodeURIComponent(this.apiKey)}`;
    const res = await fetch(url2);
  ...
  async fetchForex(pair) {
    const url2 = `${BASE_URL3}/exchange_rate?symbol=${encodeURIComponent(pair)}&apikey=${encodeURIComponent(this.apiKey)}`;
    const res = await fetch(url2);
...
// src/engine/providers/yahoo.ts
var YahooFinanceProvider = class {
  constructor(_config) {
    void _config;
  }
  async fetchQuote(symbol) {
    const url2 = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?interval=1d&range=1d`;
    const res = await fetch(url2);
  ...
  async fetchChart(symbol, interval = "1d", range = "1mo") {
    const url2 = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?interval=${interval}&range=${range}`;
    const res = await fetch(url2);
  ...
  async fetchOptions(symbol) {
    const url2 = `https://query1.finance.yahoo.com/v7/finance/options/${encodeURIComponent(symbol)}`;
    const res = await fetch(url2);
...
// src/engine/providers/fmp.ts
var BASE_URL4 = "https://financialmodelingprep.com/api/v3";
var FMPProvider = class {
  apiKey;
  constructor(config) {
    this.apiKey = config.apiKey;
  }
   ...
   */
  async fetchQuote(symbol) {
    const url2 = `${BASE_URL4}/quote/${symbol}?apikey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  async fetchNews(symbol, limit = 10) {
    const url2 = `${BASE_URL4}/stock_news?tickers=${symbol}&limit=${limit}&apikey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  async fetchProfile(symbol) {
    const url2 = `${BASE_URL4}/profile/${symbol}?apikey=${this.apiKey}`;
    const res = await fetch(url2);
   ...
   */
  async fetchTechnical(symbol, indicator = "rsi", period = 14) {
    const url2 = `${BASE_URL4}/technical_indicator/1day/${symbol}?type=${indicator}&period=${period}&apikey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  async fetchAnalystTargets(symbol) {
    const url2 = `https://financialmodelingprep.com/api/v4/price-target-consensus?symbol=${encodeURIComponent(symbol)}&apikey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  async fetchAnalystRecommendations(symbol) {
    const url2 = `${BASE_URL4}/analyst-stock-recommendations/${encodeURIComponent(symbol)}?apikey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  async fetchUpgradesDowngrades(symbol) {
    const url2 = `https://financialmodelingprep.com/api/v4/upgrades-downgrades?symbol=${encodeURIComponent(symbol)}&apikey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  async fetchStockPeers(symbol) {
    const url2 = `https://financialmodelingprep.com/api/v4/stock_peers?symbol=${encodeURIComponent(symbol)}&apikey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  async fetchHistoricalLine(symbol) {
    ...
    const url2 = `${BASE_URL4}/historical-price-full/${enc}?serietype=line&apikey=${this.apiKey}`;
    const res = await fetch(url2);
  ...
  async fetchEarningsCalendar(from, to) {
    const url2 = `${BASE_URL4}/earning_calendar?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&apikey=${this.apiKey}`;
    const res = await fetch(url2);
...
// src/engine/chartSymbolRouting.ts
function resolveChartSymbolRouting(canonical) {
...
// src/engine/services/chartService.ts
function sourceLabelFromProviderId(id) {
...
var ChartService = class {
  ...
  policy;
  chartCreds;
  constructor(cache, deduper, router, health, configs, chartCreds) {
    this.cache = cache;
    this.deduper = deduper;
    this.router = router;
    this.health = health;
  ...
  async getChart(symbol, timeframe, limit) {
  ...
  aggregateFromTo(timeframe, limit) {
    ...
    const from = new Date(Date.now() - calDays * 864e5).toISOString().split("T")[0];
    return { from, to };
  }
  timeframeToYahoo(tf, limit) {
    const map = {
      "1M": ["1m", "1d"],
      "5M": ["5m", "5d"],
      "15M": ["15m", "5d"],
  ...
  async fetchFromProviders(symbol, timeframe, limit) {
    ...
    const capableProviders = Array.from(this.providers.entries()).filter(([, config]) => config.supportedTimeframes.includes(timeframe)).map(([id]) => id);
    ...
    const priorConfigured = this.chartCreds.polygon || this.chartCreds.twelvedata || this.chartCreds.fmp;
  ...
  normalizeChart(symbol, timeframe, raw, sourceName, limit) {
    let candles = [];
    if (raw.results) {
      candles = raw.results.map((r) => ({
      ...
      candles = [...hist].reverse().map((h) => ({
        time: String(h.date ?? h.formattedDate ?? "").includes("T") ? String(h.date) : `${String(h.date ?? "")}T00:00:00.000Z`,
      ...
      candles = raw.timestamp.map((t, i) => ({
        time: new Date(t * 1e3).toISOString(),
        open: q.open[i],
        high: q.high[i],
        low: q.low[i],
        close: q.close[i],
  ...
  timeframeToMultiplier(tf) {
    const map = { "1M": 1, "5M": 5, "15M": 15, "30M": 30, "1H": 1, "4H": 4, "1D": 1 };
    return map[tf] || 1;
  }
  timeframeToTimespan(tf) {
  ...
  timeframeToTwelveDataInterval(tf) {
    const map = {
      "1M": "1min",
      "5M": "5min",
      "15M": "15min",
...
// src/engine/services/scannerService.ts
var ScannerService = class {
   ...
   */
  async scan(filter = {}) {
    const cacheKey = Normalizer.cacheKey("scanner", filter);
    const result = await this.cache.getOrFetch(
      cacheKey,
      this.policy.cacheTtlMs,
      this.policy.staleWhileRevalidate || false,
  ...
  async fetchFMPScanner(providerId, provider, _filter) {
  ...
  async fetchPolygonScanner(providerId, provider, _filter) {
...
// src/engine/services/axeService.ts
var AxeService = class {
  cache;
  deduper;
  health;
  providers;
  policy;
  chartService;
  constructor(cache, deduper, router, health, chartService, configs) {
    this.cache = cache;
    this.deduper = deduper;
    this.health = health;
   ...
   */
  async getContext(symbol, timeframe, userId) {
    const normSymbol = Normalizer.symbol(symbol);
    const normTf = Normalizer.timeframe(timeframe);
  ...
  async getStatus(userId) {
    ...
    const activeSymbols = [
      ...new Set(
        memory.map((m) => m.symbol).filter((s) => typeof s === "string" && s.length > 0)
      )
    ];
    return {
      isOnline: true,
      lastActivity: memory[0]?.createdAt || (/* @__PURE__ */ new Date()).toISOString(),
      activeSymbols,
      pendingAlerts,
  ...
  // --- Private builders ---
  async buildContext(symbol, timeframe, userId) {
  ...
  async fetchMemory(userId, symbol) {
    const config = this.providers.get("supabase_1");
    if (!config) throw new Error("No Supabase provider configured");
    try {
  ...
  calculateKeyLevels(chart) {
    ...
    const vwap = candles.slice(-20).reduce((sum, c) => sum + c.close, 0) / 20;
  ...
  calculateBias(signals, patterns) {
    let bullishScore = 0;
    let bearishScore = 0;
  ...
  buildAnalysis(symbol, bias, confidence, signals, levels, patterns) {
    const parts = [];
...
// src/engine/services/brokerTradesService.ts
function hex(bytes) {
...
var BrokerTradesService = class {
  constructor(cache, deduper, supabase2) {
    this.cache = cache;
    this.deduper = deduper;
  ...
  async createAccount(userId, args) {
  ...
  async getTradeHistory(userId, query) {
...
// src/engine/services/dashboardService.ts
var DashboardService = class {
  ...
  getDashboard() {
    ...
    return {
      ...
      credentialSlots: this.credentialSlots.length ? [...this.credentialSlots] : void 0
    };
  }
  /**
  ...
  buildProviderMetrics() {
    ...
    return configs.map((config) => {
      ...
      const cooldownRemainingSec = health?.cooldownUntil ? Math.max(0, Math.ceil((health.cooldownUntil - Date.now()) / 1e3)) : void 0;
      const lastError = health?.lastErrorMessage ? `${health.cooldownReason ? `[${health.cooldownReason}] ` : ""}${health.lastErrorMessage}` : health?.lastError ? new Date(health.lastError).toISOString() : "never";
      return {
        id: config.id,
        ...
        utilizationPercent: config.monthlyLimit > 0 ? config.usedThisMonth / config.monthlyLimit * 100 : 0,
  ...
  buildEfficiencyMetric(providers, cache) {
    ...
    const avgLatency = providers.length > 0 ? providers.reduce((sum, p) => sum + p.avgLatencyMs, 0) / providers.length : 0;
    return {
      totalRequests,
      realApiCalls,
  ...
  buildOverview(providers, efficiency) {
    const healthy = providers.filter((p) => p.isHealthy);
    const inCooldown = providers.filter((p) => p.isInCooldown);
  ...
  generateRecommendations(providers, cache, efficiency) {
    const recs = [];
    if (efficiency.creditSavingsRate > 0.5) {
      recs.push({
        priority: "info",
        message: `\u{1F4B0} Excellent credit savings: ${(efficiency.creditSavingsRate * 100).toFixed(1)}% of requests served from cache`,
        action: "Your free tiers are lasting much longer"
      });
    } else if (efficiency.creditSavingsRate < 0.2) {
      recs.push({
        priority: "warning",
        message: `\u{1F4A1} Low cache hit rate: ${(efficiency.cacheHitRate * 100).toFixed(1)}% \u2014 consider increasing cache TTL`,
    ...
    if (exhausted.length > 0) {
      for (const p of exhausted) {
        recs.push({
          priority: "critical",
          message: `\u{1F534} ${p.provider} (${p.id}) has exhausted its monthly limit (${p.monthlyLimit} calls)`,
    ...
    if (cache.size > cache.maxSize * 0.9) {
      recs.push({
        priority: "warning",
        message: `\u{1F4BE} Cache near capacity: ${cache.size}/${cache.maxSize} entries`,
    ...
    if (cache.staleServes > cache.hits * 0.1) {
      recs.push({
        priority: "info",
        message: `\u{1F504} ${cache.staleServes} stale-while-revalidate serves \u2014 background refresh is working`,
...
// src/engine/services/earningsService.ts
var EarningsService = class {
  ...
  normalize(rows) {
    return (rows ?? []).map((r) => {
      ...
      const time = timeRaw === "BMO" || timeRaw === "AMC" || timeRaw === "DMT" ? timeRaw : "TBD";
      ...
      const epsSurprise = typeof epsEstimate === "number" && typeof epsActual === "number" ? epsActual - epsEstimate : null;
      const revenueSurprise = typeof revEstimate === "number" && typeof revActual === "number" ? revActual - revEstimate : null;
      const impact = r.impact || (typeof r.marketCap === "number" && r.marketCap > 5e10 ? "high" : typeof r.marketCap === "number" && r.marketCap > 1e10 ? "medium" : "low");
      return {
        ticker,
...
// src/engine/services/contextService.ts
var ContextService = class {
  ...
  async getRelativePerformance(symbol) {
    const sym = Normalizer.symbol(symbol);
  ...
  async getSentimentShort(symbol) {
    const sym = Normalizer.symbol(symbol);
  ...
  async fetchAnalystConsensus(symbol) {
    ...
    for (const id of chain) {
      ...
      try {
        ...
        const { value: [targets, recs, quote, upgrades] } = await withRetry(
          async () => {
            ...
            return await Promise.all([
              ...
              config.provider.fetchQuote(symbol).then((q2) => [q2]).catch(() => []),
        ...
        const recentActions = (upgrades ?? []).slice(0, 12).map((u, i) => ({
        ...
        return {
          ...
          target: {
            ...
            median: t?.targetMedian != null ? Number(t.targetMedian) : void 0,
  ...
  async fetchRelativePerformance(symbol) {
    const { chain, primaryId } = this.selectChain();
    let lastError = null;
    for (const id of chain) {
      const config = this.providers.get(id);
      ...
      try {
        ...
        const quotes = await config.provider.fetchQuote(tickers.join(",")).catch(async () => {
          const url2 = `https://financialmodelingprep.com/api/v3/quote/${tickers.join(",")}?apikey=${config.provider.apiKey ?? ""}`;
          const res = await fetch(url2);
          if (!res.ok) return [];
          return res.json();
        });
        ...
        const byTicker = Object.fromEntries(quoteList.map((q) => [q.symbol, q]));
        const selected = byTicker[symbol];
        const benchQ = byTicker["SPY"];
        const peers = peersArr.map((p) => byTicker[p]).filter(Boolean).map((q) => ({
          symbol: q.symbol,
          name: q.name ?? q.symbol,
          changePercent: q.changesPercentage ?? 0,
          price: q.price ?? 0
        }));
        const sectorAverage = peers.length ? peers.reduce((sum, p) => sum + (p.changePercent || 0), 0) / peers.length : void 0;
        return {
          symbol,
          sectorName: selected?.sector ?? void 0,
          benchmark: benchQ ? { symbol: benchQ.symbol, changePercent: benchQ.changesPercentage ?? 0 } : void 0,
          sectorAverage,
          peers: [
            {
              symbol,
  ...
  async fetchKeyLevels(symbol) {
    ...
    for (const id of chain) {
      ...
      try {
        ...
        const historical = hist?.historical ?? [];
        const ath = Array.isArray(historical) ? historical.reduce((m, row) => Math.max(m, Number(row.close ?? row.price ?? 0) || 0), 0) || void 0 : void 0;
        const ma = (res, period) => {
        ...
        const levels = [];
        if (hi52) {
          levels.push({ kind: "resistance", label: "52W high", price: hi52, distancePercent: price ? (hi52 - price) / price * 100 : void 0 });
        }
        mas.forEach((m) => {
          levels.push({
            ...
            label: `${m.period}MA`,
            price: m.value,
            distancePercent: price ? (m.value - price) / price * 100 : void 0
          });
        });
        if (lo52) {
          levels.push({ kind: "support", label: "52W low", price: lo52, distancePercent: price ? (lo52 - price) / price * 100 : void 0 });
        }
        return {
          symbol,
          currentPrice: price,
          ...
          week52High: hi52,
          ath,
          drawdownFromAth: ath && price ? (price - ath) / ath * 100 : void 0,
          movingAverages: mas,
          indicators,
          levels
        };
  ...
  async fetchSentimentShort(symbol) {
    const { chain, primaryId } = this.selectChain();
    let lastError = null;
    for (const id of chain) {
      const config = this.providers.get(id);
      ...
      try {
        ...
        const { value: [profile, news] } = await withRetry(
          async () => {
            this.router.recordUsage(id, costUnits({ domain: "news", provider: "fmp", endpoint: "fetchProfile" }));
            return await Promise.all([
        ...
        this.health.recordSuccess(id);
        const shortPercent = profile && typeof profile.shortPercentFloat === "number" ? profile.shortPercentFloat : void 0;
        const shortShares = profile && typeof profile.sharesShort === "number" ? profile.sharesShort : void 0;
        ...
        const squeezeScore = shortPercent != null ? Math.max(0, Math.min(100, Math.round(shortPercent * 2))) : void 0;
        return {
          symbol,
          squeezeScore,
          shortInterest: {
...
// src/engine/services/intelService.ts
var IntelService = class {
  ...
  jetProviders;
  vesselProviders;
  constructor(cache, deduper, router, health, jets, vessels) {
    this.cache = cache;
    this.deduper = deduper;
    this.router = router;
    this.health = health;
  ...
  async getVesselStream() {
    ...
    return result.data ?? { vessels: [], alerts: [] };
  }
  selectChain(m) {
    ...
    for (const id of chain) {
      ...
      try {
        ...
        const { value: states } = await withRetry(
          async () => {
            this.router.recordUsage(id, costUnits({ domain: "intel", provider: id.split("_")[0], endpoint: "fetchStatesAll" }));
    ...
    for (const id of chain) {
      ...
      try {
        ...
        const { value: events } = await withRetry(
          async () => {
            this.router.recordUsage(id, costUnits({ domain: "intel", provider: id.split("_")[0], endpoint: "fetchSnapshot" }));
  ...
  mapAis(events) {
    ...
    for (const e of events ?? []) {
      ...
      const name = String(meta?.ShipName ?? "") || String(e?.Message?.ShipStaticData?.ShipName ?? "") || `Vessel ${mmsi}`;
      const type = String(meta?.ShipType ?? "") || String(e?.Message?.ShipStaticData?.ShipType ?? "") || "Vessel";
...
var supabase = createClient(url || "http://placeholder", key || "placeholder", {
  auth: {
...
if (import.meta.env.DEV && typeof window !== "undefined") {
  ...
  window.___supabase = supabase;
}

// src/lib/axeMt5CloudInvoke.ts
async function invokeAxeMt5Cloud(body) {
  const { data, error } = await supabase.functions.invoke("axe-mt5-cloud", { body });
  if (error) {
...
// src/engine/adapter/engineAdapter.ts
var EngineAdapter = class {
  newsService;
  contextService;
  intelService;
  macroService;
  ...
  dashboardService;
  brokerTradesService;
  constructor(newsService, contextService, intelService, macroService, accountService, chartService, scannerService, earningsService, axeService, dashboardService, brokerTradesService) {
  ...
  async macroSeries(key2, range) {
    return this.macroService.getSeries(key2, { range });
  }
  async listMacroSeries() {
  ...
  createBrokerAccount(userId, args) {
    return this.brokerTradesService.createAccount(userId, args);
  }
  createCloudMt5Connection(userId, args) {
  ...
  getTradeHistory(userId, query) {
    return this.brokerTradesService.getTradeHistory(userId, query);
  }
  async labelTrade(userId, args) {
    await this.brokerTradesService.labelTrade(userId, args);
    return { ok: true };
  }
  getJournalAnalytics(userId, query) {
    return this.brokerTradesService.getAnalytics(userId, query);
  }
};

// src/engine/providers/supabase.ts
var SupabaseProvider = class {
  ...
  async fetchAccount(userId) {
    const { data, error } = await this.client.from("accounts").select("*").eq("user_id", userId).maybeSingle();
   ...
   */
  async fetchPositions(userId) {
    const { data, error } = await this.client.from("positions").select("*").eq("user_id", userId).eq("status", "open");
   ...
   */
  async fetchWatchlist(userId) {
    const { data, error } = await this.client.from("watchlists").select("*").eq("user_id", userId);
   ...
   */
  async fetchAxeMemory(userId, symbol) {
    let query = this.client.from("axe_memory").select("*").eq("user_id", userId).order("created_at", { ascending: false });
  ...
  // ---------------------------------------------------------------------------
  async listBrokerAccounts(userId) {
    const { data, error } = await this.client.from("user_broker_accounts").select(
    ...
    return (data ?? []).map((r) => ({
      ...
      connectionMethod: r.connection_method ?? "local_bridge",
  ...
  async insertBrokerAccount(args) {
    const { data, error } = await this.client.from("user_broker_accounts").insert({
      user_id: args.userId,
    ...
    return {
      ...
      connectionMethod: data.connection_method ?? "local_bridge",
  ...
  async fetchTradeHistory(userId, query) {
    const limit = query.limit ?? 200;
    let q = this.client.from("broker_trades").select("id,user_id,account_id,external_trade_id,symbol,side,volume,open_time,close_time,open_price,close_price,pnl,fees,created_at,updated_at").eq("user_id", userId).eq("account_id", query.accountId).order("close_time", { ascending: false, nullsFirst: false }).limit(limit);
    ...
    const labelsByTradeId = {};
    if (ids.length > 0) {
      const { data: lbls, error: lerr } = await this.client.from("trade_journal_labels").select("trade_id,label,note,updated_at").eq("user_id", userId).in("trade_id", ids);
      if (lerr) throw new Error(`Supabase trade_journal_labels error: ${lerr.message}`);
      for (const r of lbls ?? []) {
        labelsByTradeId[String(r.trade_id)] = {
          label: r.label,
          note: r.note,
          updatedAt: r.updated_at
        };
  ...
  async upsertTradeLabel(args) {
    const { error } = await this.client.from("trade_journal_labels").upsert(
      {
  ...
  async getJournalAnalytics(userId, query) {
...
// src/engine/providers/finnhub.ts
var BASE_URL5 = "https://finnhub.io/api/v1";
var FinnhubProvider = class {
  ...
  async fetchNews(symbol, limit = 10) {
    ...
    const url2 = `${BASE_URL5}/company-news?symbol=${encodeURIComponent(symbol)}&from=${encodeURIComponent(fromStr)}&to=${encodeURIComponent(toStr)}&token=${encodeURIComponent(this.apiKey)}`;
    const res = await fetch(url2);
...
// src/engine/providers/newsdata.ts
var BASE_URL6 = "https://newsdata.io/api/1";
var NewsDataProvider = class {
  apiKey;
  constructor(config) {
    this.apiKey = config.apiKey;
  }
  async fetchNews(symbol, limit = 10) {
    const url2 = `${BASE_URL6}/news?apikey=${encodeURIComponent(this.apiKey)}&q=${encodeURIComponent(symbol)}&language=en`;
    const res = await fetch(url2);
...
// src/engine/providers/thenewsapi.ts
var BASE_URL7 = "https://api.thenewsapi.com/v1";
var TheNewsApiProvider = class {
  apiKey;
  constructor(config) {
    this.apiKey = config.apiKey;
  }
  async fetchNews(symbol, limit = 10) {
    const url2 = `${BASE_URL7}/news/all?api_token=${encodeURIComponent(this.apiKey)}&search=${encodeURIComponent(symbol)}&language=en&limit=${encodeURIComponent(String(Math.min(50, Math.max(1, limit))))}`;
    const res = await fetch(url2);
...
// src/engine/providers/perigon.ts
var BASE_URL8 = "https://api.goperigon.com/v1";
var PerigonProvider = class {
  apiKey;
  constructor(config) {
    this.apiKey = config.apiKey;
  }
  async fetchNews(symbol, limit = 10) {
    const url2 = `${BASE_URL8}/all?apiKey=${encodeURIComponent(this.apiKey)}&q=${encodeURIComponent(symbol)}&pageSize=${encodeURIComponent(String(Math.min(50, Math.max(1, limit))))}`;
    const res = await fetch(url2);
...
// src/engine/providers/opensky.ts
var OpenSkyProvider = class {
  username;
  password;
  constructor(config) {
...
// src/engine/providers/aisstream.ts
var AISStreamProvider = class {
  apiKey;
  constructor(config) {
    this.apiKey = config.apiKey;
  }
...
function createEngine(config) {
  ...
  const hasKeys = (keys) => keys.some((k) => k.trim().length > 0);
  ...
  const fmpProviders = (fmpKeys.length > 0 ? fmpKeys : [""]).map((apiKey) => new FMPProvider({ apiKey }));
  const fredProviders = (fredKeys.length > 0 ? fredKeys : [""]).map((apiKey) => new FREDProvider({ apiKey }));
